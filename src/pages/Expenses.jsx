/**
 * Expenses.jsx
 * Expenses page — V2 spec + inline edit for existing expenses.
 *
 * EDIT LOGIC:
 *  - All updates happen on the frontend (no separate API needed).
 *  - editExpense() maps over rawData.expenses, finds by id, replaces in-place.
 *  - Saves full rawData blob to Supabase — same as add/delete.
 *  - Dashboard recalculates automatically via useMemo on rawData change.
 */
...
function Toast({ msg, onDone }) {
  ...
  return (
    <div style={{ position:"fixed", bottom:"90px", left:"50%", transform:"translateX(-50%)",
      background:"#1a0f1e", color:"#f6f2ec", borderRadius:"99px", padding:"9px 20px",
      fontSize:"13px", fontWeight:600, zIndex:700, whiteSpace:"nowrap",
      boxShadow:"0 4px 20px rgba(0,0,0,0.25)", animation:"fadeUp .2s ease both" }}>
      {msg}
    </div>
  );
}
...
// ── EditModal ─────────────────────────────────────────────────────────────────
// Inline slide-down editor — appears directly below the expense row.
function EditRow({ expense, cards, onSave, onCancel, saving }) {
  const [form, setForm] = useState({
    ...
    card:    expense.card    ?? "",
    payType: expense.payType ?? "banking",
    recur:   expense.recur   ?? "no",
    note:    expense.note    ?? "",
  });
  ...
  return (
    <div style={{ background:"#fff8fb", border:"1.5px solid #f4b8d4", borderRadius:"11px",
      padding:"14px", marginTop:"6px", marginBottom:"4px" }}>
      <div style={{ fontSize:"0.72rem", fontWeight:700, color:"#db2777",
        letterSpacing:"0.07em", textTransform:"uppercase", marginBottom:"10px" }}>
        ✏️ Editing: {expense.name}
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"8px" }}>
        ...
        <div>
          <Label>Amount ($)</Label>
          <input style={inp} type="number" step="0.01" value={form.amt}
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"8px" }}>
        ...
        <div>
          ...
          <select style={inp} value={form.cat}
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"8px" }}>
        <div>
          ...
          <select style={inp} value={form.card}
            onChange={e => setForm(f => ({ ...f, card: e.target.value }))}>
            <option value="">Cash / Debit / Chequing</option>
            {cards.map(c => <option key={c.id} value={c.id}>{c.label ?? c.name}</option>)}
          </select>
        </div>
        <div>
          <Label>Pay Type</Label>
          <select style={inp} value={form.payType}
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"8px" }}>
        ...
        <div>
          ...
          <select style={inp} value={form.recur}
      ...
      <div style={{ display:"flex", gap:"8px", marginTop:"12px" }}>
        <button onClick={handleSave} disabled={saving || !form.name.trim() || !form.amt}
          style={{ flex:1, padding:"9px", borderRadius:"9px", border:"none",
            background: (!saving && form.name.trim() && form.amt) ? "#db2777" : "#f4b8d4",
            color:"#fff", fontFamily:"'DM Sans',sans-serif", fontWeight:700,
            fontSize:"13px", cursor: saving ? "default" : "pointer" }}>
          {saving ? "Saving…" : "✓ Save Changes"}
        </button>
        <button onClick={onCancel}
          style={{ padding:"9px 18px", borderRadius:"9px", border:"1.5px solid #fce7f3",
            background:"#fff", color:"#9b6b8a", fontFamily:"'DM Sans',sans-serif",
...
function CarryForwardPanel({ remaining, periodKey, nextPeriodLbl, rawData, onSave, saving }) {
  ...
  useEffect(() => { if (!existing) setAmt(remaining); }, [remaining]);
  ...
  function handleSave() {
    ...
    let updated = { ...rawData, carryovers: { ...(rawData.carryovers??{}), [periodKey]: co } };
    if (co.use && dest==="savings" && jarId) {
      updated = { ...updated, goals: (rawData.goals??[]).map(g =>
        String(g.id)===String(jarId) ? { ...g, saved:(Number(g.saved)||0)+co.amt } : g) };
    }
    if (co.use && dest==="debt" && cardId) {
      updated = { ...updated, cards: (rawData.cards??[]).map(c =>
        String(c.id)===String(cardId)
          ? { ...c, balance:Math.max(0,(c.balance??c.bal??0)-co.amt), bal:Math.max(0,(c.balance??c.bal??0)-co.amt) }
          : c) };
    }
    onSave(updated);
  }
        ...
        <>
          ...
          <div style={{ position:"relative" }}>
            ...
            <input style={{ ...inp, paddingLeft:"24px" }} type="number" min="0" step="0.01"
              max={remaining} value={amt} onChange={e=>setAmt(e.target.value)} />
          </div>
          {parseFloat(amt)>remaining && remaining>0 && (
            <div style={{ fontSize:"0.67rem", color:"#c24b1a", marginTop:"3px" }}>
              ⚠ Can't carry more than your leftover ({fmt(remaining)})
            </div>
          )}
...
export default function Expenses() {
  ...
  // Track which expense id is currently being edited (null = none)
  ...
  function addExpense() {
    ...
    const expense = { id:"e"+Date.now(), name:form.name.trim(), amt:parseFloat(form.amt),
      due, cat:form.cat, card:form.card, payType:form.payType, recur:form.recur, paid:false };
  ...
  // KEY FUNCTION: find by id, replace in-place — never duplicates
  function editExpense(updated) {
    const newExpenses = allExpenses.map(e => e.id === updated.id ? updated : e);
    save({ ...rawData, expenses: newExpenses });
    setEditingId(null);
    setToast("✏️ Expense updated!");
  }
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#fdf6f8", fontFamily:"'DM Sans',sans-serif",
      color:"#1a0f1e", paddingBottom:"80px" }}>
      <div style={{ maxWidth:"640px", margin:"0 auto", padding:"14px" }}>

        {/* Header */}
        <div className="fade-up" style={{ padding:"28px 0 14px" }}>
          <p style={{ fontSize:"11px", fontWeight:700, color:"#9b6b8a", letterSpacing:"0.12em",
            textTransform:"uppercase", marginBottom:"4px" }}>Budget</p>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"baseline" }}>
            <h1 style={{ fontFamily:typography.fontDisplay, fontSize:"28px", fontWeight:700,
              color:"#1a0f1e", letterSpacing:"-0.02em", lineHeight:1.1 }}>Expenses</h1>
        ...
        {error   && (
          <div style={{ background:"#fdedf1", border:"1px solid #f4a0b4", borderRadius:"14px",
            padding:"14px", marginBottom:"12px", color:"#c94d6a", fontSize:"13px" }}>⚠ {error}</div>
        )}
        ...
        {!loading && !error && (
          <>
            {/* Carryover bar */}
            {carryover > 0 && (
              <div style={{ background:"#eaf3ee", border:"1px solid #9ecab0", borderRadius:"9px",
                padding:"10px 14px", marginBottom:"10px", display:"flex",
                justifyContent:"space-between", alignItems:"center" }}>
                <div>
                  <div style={{ fontSize:"0.61rem", fontWeight:700, textTransform:"uppercase",
                    letterSpacing:"0.06em", color:"#3a6b4e" }}>Carryover from last period</div>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:"6px", marginBottom:"12px" }}>
              <button onClick={()=>moveExp(-1)} disabled={expIdx===0}
                style={{ background:"#ffffff", border:"1.5px solid #f0dce4", borderRadius:"9px",
                  padding:"7px 12px", fontWeight:700, color:"#9b6b8a",
                  cursor:expIdx===0?"not-allowed":"pointer",
                  fontFamily:"'DM Sans',sans-serif", fontSize:"14px", opacity:expIdx===0?0.4:1 }}>‹</button>
              <div style={{ flex:1, textAlign:"center", background:"#ffffff", border:"1.5px solid #f0dce4",
                borderRadius:"9px", padding:"7px 10px", fontFamily:typography.fontDisplay,
                fontWeight:700, fontSize:"14px" }}>
                {period?.lbl}
              </div>
              <button onClick={()=>moveExp(1)} disabled={expIdx===PERIODS.length-1}
                style={{ background:"#ffffff", border:"1.5px solid #f0dce4", borderRadius:"9px",
                  padding:"7px 12px", fontWeight:700, color:"#9b6b8a",
                  cursor:expIdx===PERIODS.length-1?"not-allowed":"pointer",
                  fontFamily:"'DM Sans',sans-serif", fontSize:"14px",
                  opacity:expIdx===PERIODS.length-1?0.4:1 }}>›</button>
            </div>

            {/* Yellow alert */}
            <div style={{ background:"#faf5e6", border:"1px solid #dcca84", borderRadius:"9px",
              padding:"10px 14px", marginBottom:"12px", fontSize:"0.79rem",
              color:"#7a5010", lineHeight:1.5 }}>
              💸 Paydays are the <strong>7th & 22nd</strong>. Expenses shown by due date within the period.
            </div>

            {/* Add Expense form */}
            <div style={cardStyle}>
              <div style={{ fontFamily:typography.fontDisplay, fontSize:"0.97rem",
                fontWeight:700, marginBottom:"4px" }}>+ Add Expense</div>
              <Label>Name</Label>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px" }}>
                <div><Label>Amount ($)</Label>
                  <input style={inp} type="number" placeholder="0.00" step="0.01" value={form.amt}
                    onChange={e=>setForm(f=>({...f,amt:e.target.value}))} /></div>
                <div><Label>Due Date</Label>
                  <input style={inp} type="date" value={form.due}
                    onChange={e=>setForm(f=>({...f,due:e.target.value}))} /></div>
              </div>
              <Label>Category</Label>
              ...
              <button onClick={addExpense}
                style={{ width:"100%", marginTop:"10px", padding:"10px", borderRadius:"9px",
              ...
              <div style={{ display:"flex", justifyContent:"space-between",
                ...
                <div style={{ fontFamily:typography.fontDisplay, fontSize:"0.97rem", fontWeight:700 }}>
                  ...
                  <span style={{ fontFamily:"'DM Sans',sans-serif", fontSize:"0.7rem",
                    fontWeight:400, color:"#9b6b8a", marginLeft:"4px" }}>— {period?.lbl}</span>
                </div>
                  ...
                  return (
                    <div key={e.id}>
                      ...
                      <div style={{ display:"flex", alignItems:"center", gap:"9px",
                        padding:"9px 0",
                        borderBottom: (!isEditing && i<periodExpenses.length-1) ? "1px solid #fce7f3" : "none",
                        ...
                        <input type="checkbox" checked={!!e.paid}
                          ...
                          style={{ width:"17px", height:"17px", flexShrink:0,
                            cursor:"pointer", accentColor:"#3a6b4e" }} />
                        <div style={{ flex:1, minWidth:0 }}>
                          <div style={{ fontSize:"0.83rem", fontWeight:600,
                            textDecoration:e.paid?"line-through":"none" }}>
                            {CAT_ICON(e.cat)} {e.name}
                            {linked && (
                              <span style={{ background:"#eaf1f9", color:"#2860a0", fontSize:"0.57rem",
                                fontWeight:700, letterSpacing:"0.06em", textTransform:"uppercase",
                                padding:"2px 7px", borderRadius:"5px", marginLeft:"5px" }}>
                                {linked.label??linked.name}
                              </span>
                            )}
                            {e.recur!=="no" && (
                              <span style={{ background:"#f5f0ff", color:"#7c3aed", fontSize:"0.57rem",
                                fontWeight:700, letterSpacing:"0.06em", textTransform:"uppercase",
                            ...
                            {e.note && (
                              <span style={{ fontSize:"0.67rem", color:"#9b6b8a",
                                fontWeight:400, marginLeft:"6px" }}>· {e.note}</span>
                            )}
                          </div>
                          ...
                          <div style={{ display:"flex", gap:"4px", justifyContent:"flex-end", marginTop:"2px" }}>
                            ...
                            <button
                              ...
                              style={{ background:"none", border:"none", cursor:"pointer",
                                color: isEditing ? "#db2777" : "#b8a0b8",
                                fontSize:"0.85rem", padding:"0 2px", transition:"color .15s",
                            ...
                            <button onClick={() => delExpense(e.id)}
                              style={{ background:"none", border:"none", cursor:"pointer",
                                color:"#d4b8c4", fontSize:"0.95rem", padding:"0 2px",
                      ...
                      {/* Inline edit row — slides in below the expense */}
                      {isEditing && (
                        <EditRow
                          expense={e}
                          cards={cards}
                      ...
                      {/* Separator after edit row */}
                      {isEditing && i<periodExpenses.length-1 && (
                        <div style={{ height:"1px", background:"#fce7f3", margin:"4px 0 0" }} />
                      )}
                    </div>
                  );
                })
            ...
            <div style={cardStyle}>
              <div style={{ fontFamily:typography.fontDisplay, fontSize:"0.97rem",
                fontWeight:700, marginBottom:"10px" }}>📊 Period Summary</div>
              <div style={{ background:"#fff5f9", border:"1px solid #fce7f3",
                borderRadius:"9px", padding:"13px" }}>
                <div style={{ display:"flex", justifyContent:"space-between",
                  fontSize:"0.81rem", marginBottom:"6px" }}>
                ...
                <div style={{ display:"flex", justifyContent:"space-between",
                  fontSize:"0.81rem", paddingTop:"8px", borderTop:"1px solid #fce7f3",
                  fontWeight:700 }}>
                  <span>Remaining</span>
            ...
            {totalAmt>0 && (
              <div style={{ ...cardStyle, padding:"14px 16px" }}>
                <div style={{ display:"flex", justifyContent:"space-between",
                  fontSize:"0.83rem", fontWeight:600, marginBottom:"6px" }}>
                  <span>Expense Progress</span>
                  <span style={{ color:"#9b6b8a", fontSize:"0.72rem" }}>{fmt(paidAmt)} / {fmt(totalAmt)}</span>
                </div>
                <div style={{ height:"7px", background:"#fce7f3", borderRadius:"4px",
                  overflow:"hidden", marginBottom:"6px" }}>
                  <div style={{ height:"100%",
                    width:`${totalAmt>0?(paidAmt/totalAmt*100):0}%`,
                    background:"#db2777", borderRadius:"4px",
                    transition:`width 0.6s ${transitions.spring}` }} />
                </div>
                <div style={{ display:"flex", justifyContent:"space-between",
                  fontSize:"0.67rem", color:"#9b6b8a" }}>
