  ...
  const expenses = [
    ...rawExpenses
      ...
      .map(e => {
        ...
        const y   = rs.getFullYear();
        const m   = rs.getMonth();
        const day = e.dueDay ? Math.min(e.dueDay, new Date(y, m+1, 0).getDate()) : null;
        const base = day ? new Date(y, m, day) : null;
        const dateStr = e.dueDate || e.due || e.date
          || (base ? toLocalDateStr(base >= rs ? base : new Date(y, m+1, Math.min(e.dueDay, new Date(y, m+2, 0).getDate()))) : null);
        if (!dateStr) return null;
        return {
          id:        e.id,
          name:      e.label || e.name,
          amount:    e.amount ?? e.amt ?? 0,
          date:      dateStr,
          dueDate:   dateStr,
    ...
    ...installments
      ...
      .map(i => ({
        id:        `inst-${i.id}`,
        name:      i.label,
        amount:    i.monthly ?? i.amt,
        dueDate:   i.startDate ?? i.start,
        recurring: true,
        frequency: "monthly",
        rangeStart,
        rangeEnd,
